          ...
          >
            ...
            <q-card-section class="horizontal">
              <q-select
                ...
                :change="ValidModelo()"
                :option-value="
                  (opt) => (Object(opt) === opt && 'id' in opt ? opt.id : '')
                "
                :option-label="
                  (opt) =>
                    Object(opt) === opt && 'name' in opt ? opt.name : ''
            ...
            <q-card-section class="horizontal">
              <q-input
                ...
                :rules="[
                  (val) =>
                    (val && val.length > 0) || 'Debe introducir un nombre ',
                ]"
              />
            </q-card-section>
                ...
                :option-label="
                  (opt) =>
                    Object(opt) === opt && 'name' in opt
                      ? opt.name + ' ' + opt.lastname + ' ' + opt.documento
                      : ''
                "
                emit-value
                map-options
            ...
            <q-card-section class="horizontal">
              <q-input
                ...
                :rules="[
                  (val) =>
                    (val && val.length > 0) || 'Debe introducir un correo ',
                ]"
              />
            </q-card-section>
            <q-card-section class="horizontal">
            ...
            <q-card-section
              class="horizontal"
              v-if="validHoster & validMarca & validModelo"
            >
              <q-select
                filled
                ...
                :option-label="
                  (opt) =>
                    Object(opt) === opt && 'username' in opt ? opt.username : ''
                "
                emit-value
                map-options
              />
      ...
      <div class="q-pa-md q-gutter-sm">
        <q-dialog v-model="masivo" persistent>
          ...
          >
            ...
            <q-card-section class="horizontal">
              <p>
                El Excel debe tener el siguiente formato: 1- FECHA_VENTA 2-
                MODELO 3- CHASIS 4- COLOR 5- NOMBRE_COMPLETO_CLIENTE 6-
                EMAIL_CLIENTE 7- DOCUMENTO_CLIENTE 8- TELEFONO_CLIENTE 9-
                DOCUMENTO_VENDEDOR 10- PRECIO_REGULAR 11- PRECIO_VENTA
              </p>
              <div>
                <!-- Enlace simple -->
                <a href="/ventas.xlsx" target="_blank">Plantilla </a>
                <!-- Botón de Quasar -->
              </div>
              <p>Formato Fecha Excel: YYY-MM-DD</p>
            </q-card-section>
            <q-separator />
  ...
  methods: {
    ...
    SelectedMarcaInput() {
      ...
      if (select) {
        ...
        var selectedModelo = this.modelosFiltrados.find(
          (y) => y.id === this.vehiculo.model
        );
        if (!selectedModelo) {
          this.vehiculo.model = '';
        }
    ...
    async AddVenta() {
      if (
        ...
        const foundModelo = this.modelos.find(
          (x) => x.id === this.vehiculo.model
        ).name;
        ...
        const VentaObjeto = {
          ...
          fecha: new Date(this.venta.fecha).toISOString(),
        ...
        await api
          ...
          .then((response) => {
            this.$q.notify({
              ...
              position: 'top',
              message:
                'Se editó la venta del vehiculo ' +
                response.data.vehiculo.chasis,
              icon: 'done',
            });
          })
    ...
    async EditVenta() {
      if (
        ...
        await api
          .patch('/venta/' + this.idselected, VentaObjeto, {
            headers: {
              Authorization: `Bearer ${authenticate.getUserLogged()}`,
            },
          })
      ...
      await api
        ...
        .then((response) => {
          this.$q.notify({
            ...
            position: 'top',
            message:
              'Se Desahabilito el venta del sistema ' + response.data.name,
            icon: 'done',
          });
        })
    ...
    exportTable() {
      ...
      const status = exportFile('Listado_Ventas.csv', content, 'text/csv');
    ...
    async AddVentaMasiva() {
      if (this.excelValido) {
        await api
          .post('/venta/masivo/', this.ventasMasivas, {
            headers: {
              Authorization: `Bearer ${authenticate.getUserLogged()}`,
            },
          })
    ...
    validarDatos(datos) {
      ...
      datos.forEach((element) => {
        if (
          element.CHASIS == '' ||
          typeof this.colores.find((color) => color.name === element.COLOR) ==
            'undefined' ||
          element.FECHA_VENTA == '' ||
          element.NOMBRE_COMPLETO_CLIENTE == '' ||
          element.PRECIO_REGULAR == '' ||
          element.PRECIO_VENTA == '' ||
          typeof this.modelos.find(
            (modelo) => modelo.name === element.MODELO.toString()
          ) === 'undefined' ||
          typeof this.vendedores.find(
            (vendedor) =>
              vendedor.documento === element.DOCUMENTO_VENDEDOR.toString()
          ) === 'undefined'
        ) {
          valor = false;
          this.$q.notify({
            message:
              'Esta Venta tiene los campos mal: ' +
              'Chasis: ' +
              element.CHASIS +
              '-' +
              'Cliente:' +
          ...
          console.log(element.FECHA_VENTA);
          const fechaObjeto = new Date((element.FECHA_VENTA - 25569) * 86400000)
            .toISOString()
            .split('T')[0];

          console.log(fechaObjeto);
          this.ventasMasivas.push({
            nombreCliente: element.NOMBRE_COMPLETO_CLIENTE,
            telefonoCliente: element.TELEFONO_CLIENTE,
            correoCliente: element.EMAIL_CLIENTE,
            documentoCliente: element.DOCUMENTO_CLIENTE,
            fecha: fechaObjeto,
            chasis: element.CHASIS,
            modelo: element.MODELO,
            precioVenta: element.PRECIO_REGULAR,
            precioFinVenta: element.PRECIO_VENTA,
            idVendedor: element.DOCUMENTO_VENDEDOR,
            color: element.COLOR,
          });
        }
      });
    ...
    filterFn(val, update) {
      ...
      update(() => {
        ...
        this.optionsVendedores = this.optionsVendedores.filter(
